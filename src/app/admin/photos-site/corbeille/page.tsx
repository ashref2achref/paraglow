...
function slotLabel(slotKey: string) {
  return MEDIA_SLOTS.find((s) => s.key === slotKey)?.label || slotKey
}

export default function PhotosSiteCorbeillePage() {
  ...
  const handleRestore = async (ids: string[]) => {
    try {
      ...
      if (res.ok) {
        toast.success(`${ids.length} média(s) restauré(s) avec succès !`)
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      if (deleteMode === 'empty') {
        const expectedCount = media.length
        const res = await fetch('/api/admin/media/trash', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        const ids = deleteMode === 'single' ? [activeMediaId!] : selectedIds
        const res = await fetch('/api/admin/media/trash', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (res.ok) {
          ...
          if (deletedCount !== ids.length) {
            ...
            toast.success(`${ids.length} média(s) supprimé(s) définitivement.`)
          }
          setSelectedIds([])
          await loadTrash()
        } else {
    ...
    <div className="space-y-6 font-sans text-[#2a1f0e]">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#FBF6EC] border-b border-[#eadfca] text-xs font-semibold uppercase tracking-wider text-[#6b5f4f]">
                  ...
                  <th className="py-4 px-3 w-16">Aperçu</th>
              ...
              <tbody className="divide-y divide-[#ede8de] text-xs">
                {media.map((m) => (
                  <tr key={m.id} className="hover:bg-[#FBF6EC]/10 transition-colors">
                    ...
                    <td className="py-3.5 px-4 max-w-[260px]">
                      <div className="font-semibold text-sm text-[#153f2b] line-clamp-1">{slotLabel(m.slotKey)}</div>
                      <div className="text-[10px] text-[#9b8f7a] font-mono mt-0.5">{m.slotKey}</div>
                    </td>
                    <td className="py-3.5 px-4 text-[#6b5f4f]">
                      {m.supprimeLe ? new Date(m.supprimeLe).toLocaleString('fr-FR') : 'Date inconnue'}
                    </td>
                    <td className="py-3.5 px-4 text-center space-x-1.5 whitespace-nowrap">
                      ...
                      <button
                        onClick={() => { setActiveMediaId(m.id); setDeleteMode('single'); setIsDeleteConfirmOpen(true) }}
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-[#1b3a1e] text-white border border-[#c9a052]/30 px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-6 z-50 text-xs md:text-sm font-sans max-w-[90%] md:max-w-2xl">
          <span className="font-semibold">{selectedIds.length} média(s) sélectionné(s)</span>
      ...
      >
        <div className="space-y-4 text-xs text-left text-[#2a1f0e]">
          <p className="leading-relaxed">
            {deleteMode === 'empty'
              ? 'Êtes-vous sûr de vouloir vider TOUTE la corbeille ? Tous les fichiers seront définitivement effacés du serveur.'
              : deleteMode === 'bulk'
              ? `Voulez-vous supprimer définitivement ces ${selectedIds.length} média(s) ?`
